    ...
    <n-card class="glass-card hero-strip" :bordered="false">
      <div class="panel-heading">
        ...
        <div class="panel-toolbar">
          ...
          <n-button type="primary" :loading="loading" @click="loadAdmins(true)">{{ t('admins.refresh') }}</n-button>
          <n-tag round>{{ t('admins.total', { count: admins.length }) }}</n-tag>
        </div>
      </div>
    </n-card>

    <div class="summary-metric-grid admins-summary">
      <n-card
...
const summaryCards = computed(() => {
  ...
  return [
    { key: 'total', label: t('admins.title'), value: String(list.length), icon: 'solar:users-group-rounded-bold', tone: 'green' },
    { key: 'root', label: t('admins.rootAdmin'), value: String(root), icon: 'solar:crown-bold', tone: 'amber' },
    { key: 'group', label: t('admins.source'), value: t('admins.groupCount', { n: group }), icon: 'solar:chat-round-dots-bold', tone: 'blue' },
    { key: 'manual', label: t('admins.note'), value: t('admins.kvCount', { n: manual }), icon: 'solar:key-bold', tone: 'orange' },
  ];
});
...
async function loadAdmins(force = false) {
  loading.value = true;
  try {
    const data = await fetchAdmins(limit.value || 50, { force });
