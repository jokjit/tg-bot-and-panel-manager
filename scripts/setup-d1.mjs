...
参数：
  ...
  --remote          创建后立即应用远程迁移
  --skip-migrate    只创建数据库并回填 wrangler.toml，不执行迁移
  --dry-run         仅打印步骤，不真正执行
 --help            查看帮助

说明：
  - 运行前请先完成 wrangler login，或已配置 Cloudflare 相关环境变量。
  - 脚本会自动尝试把 D1 绑定写回 wrangler.toml。
`);
}
...
async function main() {
  ...
  const createResult = runCommand(
    'npx',
    ['wrangler', 'd1', 'create', args.databaseName],
    { capture: true, dryRun: args.dryRun },
  );

  if (!args.dryRun) {
    const output = `${createResult.stdout || ''}${createResult.stderr || ''}`;
    const databaseId = parseDatabaseId(output);
    if (!databaseId) {
      throw new Error(`无法从 wrangler 输出中解析 D1 database_id：\n${output}`);
    }

    const wranglerContent = readFileSync(wranglerPath, 'utf8');
    const nextContent = upsertD1Binding(wranglerContent, args.binding, args.databaseName, databaseId);
    writeFileSync(wranglerPath, nextContent, 'utf8');
    console.log(`已写入 D1 绑定到：${wranglerPath}`);
    console.log(`database_id：${databaseId}`);
  } else {
    console.log(`将会把 [[d1_databases]] 写入 ${wranglerPath}`);
  }
  ...
  if (!args.skipMigrate) {
    ...
    runCommand('npx', migrateArgs, { dryRun: args.dryRun });
  }
